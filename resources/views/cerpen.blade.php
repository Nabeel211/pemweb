...
<div style="position: relative; z-index: 1;">
    <h1 class="text-center mb-4" style="font-weight: bold;">Cerpen</h1>
    ...
    <div class="comic-wrapper p-4">
        <div class="row justify-content-center">
            @foreach ($cerpenList as $cerpen)
                <div class="text-center mb-4 comic-card d-inline-block" style="width: 20%;">
                    <img src="{{ asset('img/' . $cerpen['img']) }}" alt="{{ $cerpen['title'] }}">
                    <div class="comic-details mt-2 px-2 py-2">
                        <div class="comic-title">{{ $cerpen['title'] }}</div>
                        <div class="comic-info">Chapter {{ $cerpen['chapter'] }}</div>
                        <div class="comic-info">Rating {{ $cerpen['rating'] }}</div>
                    </div>
                </div>
            @endforeach
        </div>
        ...
        <div class="d-flex justify-content-end mt-4">
            {{ $cerpenList->links('vendor.pagination.bootstrap-5') }}
        </div>
    </div>
</div>
@endsection
